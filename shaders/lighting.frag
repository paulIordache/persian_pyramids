...
float computeShadow(DirLight dirLight) {
...
void main( )
{
    float shadow = computeShadow(dirLight);
...
vec3 CalcDirLight( DirLight light, vec3 normal, vec3 viewDir )
{
    ...
    // Combine results
    vec3 ambient = light.ambient * vec3( texture( material.diffuse, TexCoords ) ) * light.lightColor;
